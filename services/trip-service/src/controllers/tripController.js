...
async function create(req, res) {
  try {
    const { fromCity, toCity, tripDate, departureTime, totalSeats, tripType, vehicleModel, baseFare, venmoHandle, zelleHandle, vehicleMpg } = req.body
    ...
    const trip = await createTrip({
      ...
      venmoHandle, zelleHandle: zelleHandle,
      vehicleMpg,
    })
...
async function join(req, res) {
  try {
    ...
    const trip = await getTripById(req.params.id)
    return res.status(200).json({ message: 'Joined trip successfully', trip })
  } catch (err) {
